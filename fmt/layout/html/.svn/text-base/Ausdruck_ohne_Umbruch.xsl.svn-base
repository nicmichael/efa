...
<!-- Stylesheet für Ausruck (Farbe), Nicolas Michael, 2002 -->
...
<xsl:output method="html" encoding="ISO-8859-1"/>
  ...
  <xsl:template match="name">
    <td width="400"><xsl:value-of select="translate(.,' ','&#160;')"/></td>
  </xsl:template>
  ...
  <xsl:template match="gruppenName">
    ...
    <tr><th align="left" colspan="2" bgcolor="#ddddff">
  ...
  <xsl:template match="wettEintrag">
    ...
    <xsl:choose>
      <xsl:when test="wettDetail">
        <td><table border="1">
	  <tr><td colspan="6"><font color="green"><b><xsl:value-of select="wettName"/></b></font><xsl:apply-templates select="wettJahrgang"/>: <xsl:value-of select="wettKilometer"/> Km<xsl:apply-templates select="wettZusatz"/></td></tr>
      ...
      <xsl:otherwise> <!-- keine Details, oder nicht erfüllt -->
        <xsl:choose>
          <xsl:when test="@erfuellt='true'">
            <td><font color="green"><b><xsl:value-of select="wettName"/></b></font><xsl:apply-templates select="wettJahrgang"/>: <xsl:value-of select="wettKilometer"/> Km<xsl:apply-templates select="wettZusatz"/></td>
          </xsl:when>
          <xsl:otherwise> <!-- nicht erfüllt -->
            <td>
	      <font color="red"><b><xsl:value-of select="wettName"/></b></font>
	      <xsl:apply-templates select="wettJahrgang"/>: 
	      <xsl:choose>
	        <xsl:when test="wettKilometer &lt; ../gruppenName/gruppenBed/@wert1"><font color="red"><xsl:value-of select="wettKilometer"/> Km</font></xsl:when>
  ...
  <xsl:template match="wettZusatz">
    <xsl:text>; </xsl:text>
    <xsl:choose>
      <xsl:when test=". &lt; ../../gruppenName/gruppenBed/@wert2"><font color="red"><xsl:value-of select="."/></font></xsl:when>
