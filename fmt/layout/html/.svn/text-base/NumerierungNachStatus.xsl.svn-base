...
<!-- Stylesheet für das Erstellen einer Kilometerliste für Mitglieder,
     die für jeden Status eines Mitglieds einzeln numeriert ist, d.h.
     Junioren, Senioren und Gäste werden einzeln numeriert.
     Für diese Auswertung muß die Ausgabe des Felds "Status" aktiviert
     sein.
     Nicolas Michael, 2006 -->
...
<xsl:output method="html" encoding="ISO-8859-1"/>
  ...
  <xsl:template match="nr">
	  ...
	  <td></td>
        ...
        <xsl:otherwise>
          <td><xsl:value-of select="count(../preceding-sibling::node()[status=$status])+1"/></td>
	</xsl:otherwise>
      </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="wettEintrag">
    ...
    <xsl:choose>
	  ...
	  <tr><td colspan="6" bgcolor="#00ff00"><b><xsl:value-of select="wettName"/></b><xsl:apply-templates select="wettJahrgang"/>: <xsl:value-of select="wettKilometer"/> Km<xsl:apply-templates select="wettZusatz"/></td></tr>
      ...
      <xsl:otherwise> <!-- keine Details, oder nicht erfüllt -->
        <xsl:choose>
          <xsl:when test="@erfuellt='true'">
            <td bgcolor="#00ff00"><b><xsl:value-of select="wettName"/></b><xsl:apply-templates select="wettJahrgang"/>: <xsl:value-of select="wettKilometer"/> Km<xsl:apply-templates select="wettZusatz"/></td>
          </xsl:when>
