...
public abstract class DataAccess implements IDataAccess {
    ...
    public static IDataAccess createDataAccess(StorageObject persistence, int type, String storageLocation,
                                               String storageUsername, String storagePassword,
                                               String storageObjectName, String storageObjectType,
                                               String storageObjectDescription) {
        IDataAccess dataAccess = null;
        switch (type) {
            case IDataAccess.TYPE_FILE_XML:
                dataAccess = (IDataAccess) new XMLFile(storageLocation, storageObjectName, storageObjectType,
            ...
            case IDataAccess.TYPE_EFA_CLOUD:
                ...
                try {
                    dataAccess = new EfaCloudStorage(efaCloudURL, storageLocation, storageUsername, storagePassword,
                            storageObjectName, storageObjectType, storageObjectDescription);
                } catch (EfaException e) {
                    Logger.log(Logger.ERROR, Logger.MSG_DATA_DATAACCESS,
                            "DataAccess initialization for " + storageObjectName + "." + storageObjectType + " (type " +
            ...
            case IDataAccess.TYPE_EFA_REMOTE:
                dataAccess = (IDataAccess) new RemoteEfaClient(storageLocation, storageUsername, storagePassword,
                        storageObjectName, storageObjectType, storageObjectDescription);
                dataAccess.setPersistence(persistence);
                return dataAccess;
        }
        Logger.log(Logger.ERROR, Logger.MSG_DATA_DATAACCESS,
                "DataAccess for " + storageObjectName + "." + storageObjectType + " (type " + type + ") is null");
        return null;
    }
    ...
    public void registerDataField(String fieldName, int dataType) throws EfaException {
        if (fieldTypes.containsKey(fieldName)) {
            throw new EfaException(Logger.MSG_DATA_GENERICEXCEPTION,
                    getUID() + ": Field Name is already in use: " + fieldName, Thread.currentThread().getStackTrace());
        }
    ...
    public int getFieldType(String fieldName) throws EfaException {
        ...
        if (i == null) {
            throw new EfaException(Logger.MSG_DATA_FIELDDOESNOTEXIST,
                    getUID() + ": Field Name does not exist: " + fieldName, Thread.currentThread().getStackTrace());
        }
        return i.intValue();
    }
    ...
    public DataKey getUnversionizedKey(DataKey key) {
        ...
        return new DataKey(key,
                bUnversionized); // this is the corresponding "unversionized" key (i.e. key with only unversionized
        // fields)
    }
    ...
    public void saveToXmlFile(String filename) throws EfaException {
        if (!isStorageObjectOpen()) {
            throw new EfaException(Logger.MSG_DATA_SAVEFAILED,
                    LogString.fileWritingFailed(filename, storageLocation, "Storage Object is not open"),
    ...
    public BackupMetaDataItem saveToZipFile(String dir, ZipOutputStream zipOut) throws EfaException {
        if (!isStorageObjectOpen()) {
            throw new EfaException(Logger.MSG_DATA_SAVEFAILED,
                    LogString.fileWritingFailed("ZIP Buffer", storageLocation, "Storage Object is not open"),
                    Thread.currentThread().getStackTrace());
        }
        ...
        try {
            ...
            metaData = new BackupMetaDataItem(getStorageObjectName(), getStorageObjectType(), zipFileEntry,
                    getStorageObjectDescription(), getNumberOfRecords(), getSCN());
            XMLFile.writeFile(this, zipOut);
        } catch (Exception e) {
            throw new EfaException(Logger.MSG_DATA_SAVEFAILED,
                    LogString.fileWritingFailed("ZIP Buffer", storageLocation, e.toString()),
                    Thread.currentThread().getStackTrace());
        } finally {
            if (lock >= 0) {
    ...
    public synchronized void copyFromDataAccess(IDataAccess source) throws EfaException {
        ...
        try {
            ...
            setInOpeningStorageObject(
                    true); // don't update LastModified Timestamps, don't increment SCN, don't check assertions!
            if (recordList.size() > 0) {
                addAll(recordList.toArray(new DataRecord[0]), -1);
            }
        } catch (Exception e) {
            throw new EfaException(Logger.MSG_DATA_COPYFROMDATAACCESSFAILED,
                    getUID() + ": Restore from DataAccess failed", Thread.currentThread().getStackTrace());
        } finally {
